  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(inputId = "site_select", label = h4("Select site"), 
                  ...
                  selectize = FALSE),
      # another one for sub_category
      selectInput(inputId = "sub_category_select", label = h4("Select scoring category"), 
                  choices = unique(MPS_tracker_data$sub_category), 
...
server <- function(input, output) {
  ...
  line_dat <- reactive({
    ...
    #   need(length(input$sub_category_select) > 0, "Please select a scoring catagory")
    # )
    # 
    MPS_tracker_data |> 
    filter(site %in% c(input$site_select), # will add user interactivity here 
           sub_category %in% c(input$sub_category_select))   # and here ... think about how to add multiple at once??
  })
  ...
  #  scale_x_continuous(limits = c(min(MPS_tracker_data$year), max(MPS_tracker_data$year))) +
    ...
    labs(title = paste0(input$sub_category_select," at ", input$site_select, " protected site"))
  })
}
